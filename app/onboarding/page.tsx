...
export default function OnboardingPage() {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      const result = await createUserProfile(deviceId, strengthsData)
      ...
      const strengthsForLocalStorage = selectedThemes.map((theme, index) => ({
        theme_id: theme.id,
        theme_name: theme.name,
        rank: index + 1,
      }))

      localStorage.setItem("user_strengths", JSON.stringify(strengthsForLocalStorage))
      localStorage.setItem("onboarding_completed", "true")
      localStorage.setItem("device_id", deviceId)

      console.log("[v0] Successfully saved strengths to database and localStorage:", strengthsForLocalStorage)
      router.push("/")
    } catch (error) {
